...
function generateSeedKeys(keysSeedBinarySize) {
    const keysSeedBinary = getRandomBytes(keysSeedBinarySize);
...
function generateSeedKeysWeb(size) {
    const seed = getRandomBytes(size);
    const { secretSpendKey, publicSpendKey, } = keysFromDefault(seed, size);
    return {
        secretSpendKey,
        publicSpendKey,
    };
}
exports.generateSeedKeysWeb = generateSeedKeysWeb;
function getRandomBytes(numBytes) {
    ...
    return typeof window !== 'undefined' && window.crypto?.getRandomValues(array)
        ? Buffer.from(array)
        : (0, crypto_1.randomBytes)(numBytes);
}
...
function generateSignature(message, privateKey, pubKey) {
    const h = fastHash(message);
    const s = (0, helpers_1.decodeScalar)(privateKey);
    ...
    while (true) {
        const k = (0, helpers_1.decodeInt)(getRandomScalar((0, crypto_1.randomBytes)(32), 32));
        const K = crypto_data_1.ec.curve.g.mul(k);
        const buf = {
            h: h,
            key: pubKeyBuf,
            comm: (0, helpers_1.encodePoint)(K),
        };
        const bufForHash = Buffer.concat([buf.h, buf.key, buf.comm]);
        const hashFromBuffer = hashToScalar(bufForHash);
        const c = (0, helpers_1.decodeInt)(hashFromBuffer);
        if (c.isZero()) {
            continue;
        }
        const r = k
        ...
        const encodedC = c.toArrayLike(Buffer, 'le', 32);
        const encodedR = r.toArrayLike(Buffer, 'le', 32);
...
function checkSignature(message, publicKey, signature) {
    try {
        const r = (0, helpers_1.decodeScalar)(signature.r);
        const c = (0, helpers_1.decodeScalar)(signature.c);
        const P = (0, helpers_1.decodePoint)(publicKey);
        const h = fastHash(message);
        const B = crypto_data_1.ec.curve.g;
        const R = P.mul(c).add(B.mul(r));
        const bufComm = (0, helpers_1.encodePoint)(R);
        const buf = {
            h,
            key: publicKey,
            comm: bufComm,
        ...
        const bufForHash = Buffer.concat([buf.h, buf.key, buf.comm]);
        const hashFromBuffer = hashToScalar(bufForHash);
        const calculatedC = (0, helpers_1.decodeInt)(hashFromBuffer);
...
function getRandomScalar(aPart, keysSeedBinarySize) {
    const tmp = Buffer.alloc(64).fill(0);
    if (!(tmp.length >= keysSeedBinarySize)) {
        throw new Error('size mismatch');
    }
